...
students.dropna(how='any')                              # Drop NaN if any NaN values are present
students.dropna(how='all')                              # Drop NaN if all NaN values are present
students.dropna(subset=['name','college'])              # Drop NaN if NaN is present in 'name' and 'college' column
students.dropna(thresh=3)                               # Drop NaN is NaN is more than 3
```
